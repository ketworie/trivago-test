...
private val urlPattern = "^https?://(www\\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\\.[a-zA-Z0-9()]{1,6}\\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)$".toRegex()
...
fun Application.configureHTTP() {
    ...
    install(RequestValidation) {
        validate(AccommodationDTO::class) {
            ...
            val isNewLocation = it.location.id == 0L
            val zipDigitsNumber = log10(abs(it.location.zipCode.toDouble())).toInt() + 1
            if (isNewLocation && zipDigitsNumber != 5) {
